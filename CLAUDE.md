...
### ORB strategy alert watcher (Pushover, added 2026-07-22)

`tv orb-alert check` (`src/core/orb_alert.js`) is a second, simpler standalone Pushover alert path, purpose-built for the `indicators/orb-ema-trend-strategy.pine` strategy (see `ORB_STRATEGY_SPEC.md`). Unlike `trade-alert`, it makes **no LLM call** — the Pine strategy's own tuned, deterministic entry logic (opening-range breakout + EMA + daily-trend filter) *is* the signal, so "a good buy-in moment" means "the strategy just placed a real order," not a separate confidence judgment. This makes it fast (a few seconds per symbol) and free to run often.

For each symbol in `ORB_ALERT_SYMBOLS` (comma-separated, default `COMEX:GC1!` — the best-backtested market as of writing), it: sets the chart to that symbol at 15m (the timeframe the strategy was tuned/backtested on), ensures "ORB EMA Trend [Keagan]" is on the chart (adds it via `chart.manageIndicator` if missing — e.g. after a TradingView relaunch), reads the strategy's `reportData().filledOrders` array via `Runtime.evaluate` (every order fill, entries *and* exits, in chronological order — not the same as `reportData().trades`, which only contains *closed* round-trips and would miss a still-open entry), and diffs the count against the last-seen count per symbol in `~/.tradingview-mcp/orb_alert_state.json`. New entries (`o.e === true`) trigger a Pushover push with direction/price/qty, a chart screenshot attached the same way `trade-alert` does, and a cooldown (`ORB_ALERT_COOLDOWN_MINUTES`, default 15) per symbol+direction. On the very first run for a symbol (no prior state), the current order count is taken as the baseline rather than back-alerting on the whole backtest history. Schedule it with `scripts/com.tradingview-mcp.orb-alert.plist.example`, same launchd pattern as `trade-alert`. Use `--dry-run` to verify the detection/message logic without sending or updating state — confirmed working end-to-end 2026-07-22 by simulating a historical order as "new" (correctly detected and formatted, dry-run so nothing sent) and separately confirming raw Pushover connectivity with a real, clearly-labeled test push.
...
- **Pushing a genuinely *new* Pine script (as opposed to editing an already-open one) is currently unreliable end-to-end (found 2026-07-16, cost a full session to characterize).** Symptoms and root causes found so far:
  ...
  - **Found 2026-07-21: TradingView's default futures margin requirement silently blocks every `strategy.entry()` call if it exceeds `initial_capital`, with no error, warning, or visible signal anywhere — the Strategy Tester panel just keeps saying "This report requires trade data" forever.** Root-caused by adding temporary `plot(..., display=display.data_window)` cumulative counters that traced the entry-condition funnel across the whole loaded history: the condition gating `strategy.entry()` was true 100+ times, `strategy.position_size` never changed once. Ruled out (in this order — cheapest/most-likely-looking theories first) an over-strict entry filter and fractional contract quantities before landing on margin. Fix: add `margin_long=0, margin_short=0` to the `strategy()` declaration — defensible for any strategy that always flattens intraday (no realistic overnight margin exposure), not a general-purpose fix for strategies that hold positions. **Do not trust chart-drawn entry labels (`label.new()` inside an `if longCondition` block) as evidence a trade actually filled** — they render regardless of whether the paired `strategy.entry()` call was accepted; they were observed rendering correctly on the chart in this session while `reportData()` showed zero real trades underneath. The only reliable ground truth for "did a trade actually happen" is `cw.getStudyById(id).study().reportData().performance.all.totalTrades` (via `Runtime.evaluate`) or the Strategy Tester's own fully-settled report — never the visible chart, and not even the tester panel's numbers without settling first (see next point).
  - **`reportData()` itself can return transient, wildly wrong values for several seconds after a symbol switch or script edit** — observed a stable-for-multiple-consecutive-polls reading of a large real profit immediately followed by a stable-for-multiple-consecutive-polls reading of a multi-million-dollar loss with an absurd (300,000+) max-contracts-held, before the actual settled value emerged. A handful of consistent reads is not proof of settlement. **Poll to stability with at least 5–6 consecutive identical reads spaced several seconds apart** before trusting any `reportData()` number, especially right after changing the symbol.
  - **Risk-%-of-equity position sizing (`riskDollars / (stopDistance × pointValue)`) has no natural upper bound** — once margin isn't blocking oversized orders (see above), an unusually tight stop distance on any given bar can produce an absurd quantity (observed: 263,920 shares on a stock, from an otherwise-correct sizing formula). Always pair risk-% sizing with an explicit `math.min(sizeFromRiskPct, maxReasonableCap)` safety ceiling, independent of the risk-% math, as a circuit breaker — don't assume "the math is right" implies "the output is sane."
  - **`tv symbol --set` on some CME futures accepts an exchange-qualified ticker that doesn't actually exist and reports success anyway** — e.g. `CME_MINI:YM1!` (Dow futures) silently produced a real-looking `state`/`chart_ready` response, but the chart itself showed "This symbol doesn't exist"; the correct prefix was `CBOT_MINI:YM1!`. Worse: `reportData()` queried in that broken state returned a **stable-looking, multi-consecutive-poll-identical result that was actually stale data left over from the previous (valid) symbol**, not an error and not zeros — nothing distinguished it from a real settled reading except that it didn't match a fresh screenshot of the chart. **After every symbol switch, verify the chart actually resolved (`ui eval` checking the visible symbol/exchange text, or a screenshot) before trusting any `reportData()` read that follows** — polling to stability is necessary but not sufficient if the symbol itself silently failed to resolve. For continuous futures with ambiguous exchange prefixes, setting the *bare* ticker (e.g. `6E1!` instead of guessing `CME:6E1!`) and letting TradingView auto-resolve the exchange was more reliable than guessing the prefix.
  - **The chart's *resolution* can silently drift independently of the symbol during a long automation session** — mid-sweep across many `tv symbol --set` calls, the timeframe silently changed from 15 to 5 with no tool call requesting it (cause unconfirmed — possibly a stray keystroke reaching the chart from an unrelated `ui type`/`ui mouse` call earlier in the session). `reportData()` kept returning perfectly stable, plausible-looking numbers for the new (wrong) resolution — nothing flagged it as inconsistent with prior runs on the same symbol. **Check `tv state`'s `resolution` field, not just `symbol`, before trusting a `reportData()` read** — a symbol check alone isn't sufficient.
  - **A specific "ghost" reading recurs across unrelated symbols during `reportData()` polling: `{trades: 31, netProfit: -61.9154, winRate: 0, profitFactor: 0}`** (or very close variants). It surfaced for two completely different symbols in the same session, each time sandwiched between otherwise-consistent real readings, never as the final settled value. Root cause unconfirmed (possibly some kind of default/placeholder state the report briefly returns during an internal recalculation restart) — but if this exact shape of result appears once amid an otherwise-stable polling sequence, treat it as noise and keep polling rather than as a real result.
  - **`screenshot` can return a stale cached image** — twice in one session, a screenshot taken immediately after a `tv symbol --set` call rendered a *byte-identical* image to an unrelated screenshot taken several actions earlier (same stale symbol, same stale Strategy Tester trade rows, down to the exact PnL figures), even though `tv state` already reported the new, correct symbol at the time. Waiting longer (~2s+) before the next screenshot, or just trusting `tv state`/`ui eval` DOM queries over a screenshot for symbol verification, avoided the problem — don't treat a screenshot taken immediately after a state change as authoritative without a beat of delay first.
