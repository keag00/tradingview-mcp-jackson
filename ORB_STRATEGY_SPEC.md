...
**Now scheduled and running continuously** via `launchctl` (`~/Library/LaunchAgents/com.tradingview-mcp.orb-alert.plist`, checking every 5 minutes) — currently watching `OANDA:GBPUSD` and `CME_MINI:ES1!` (see the GBPUSD and ES1! sections below for why those symbols are there despite bad backtests — both were explicit Keagan requests, disclosed honestly rather than silently omitted). Each alert message now includes the strategy's live win rate/profit factor for that symbol, with an explicit caution note when the historical win rate is below breakeven. The plist points at this worktree's directory, not the main checkout, since this code isn't merged yet — it'll keep working as long as the worktree exists, but needs repointing (and reloading) once the PR is merged and the worktree is cleaned up.
...
## ES1! (S&P 500 E-mini futures) — reconfirmed loser, honest disclosure (2026-07-23)

Keagan asked for a fresh ES1! backtest ahead of trading tomorrow's NY open, plus ideas for a higher win rate. Ran the current live-tuned settings (`orMinutes=5, emaLen=20, requireHold=false, minBreakoutBufferMult=0.1, riskPct=0.5, rrMultiple=2.5, atrBufferMult=0.15`) on `CME_MINI:ES1!` 15m, polled `reportData()` to stability (5+ consecutive identical reads): **50 trades, 26.0% win rate, net −$5,137.50 (−20.55%), profit factor 0.70.** This matches (doesn't improve on) the −20% net result already recorded in the 16-market sweep above — ES1! has now been a net loser in every version of this strategy tested (v1: −58%, v2/v3: −20%, today's reconfirm: −20.55%).

**Diagnosis, not just the number:** long/short split was 49 long / 1 short (the daily-EMA filter is correctly blocking almost all shorts — ES1!'s daily trend has been up, this is the filter working as designed, not a bug). The long side alone: 26.5% win rate, avg win $936 vs avg loss $468 (a genuinely good 2:1 R:R ratio) — but the breakeven win rate for a 2:1 payoff is ~33%, and this is well under that. **The problem is entry quality, not stop/target sizing.** `maxContractsHeld` stayed at 1 the whole backtest, so — unlike the GBPUSD confound above — this is *not* a position-sizing artifact; it's a real read on the entry logic.

Candidate causes, reasoning from the actual entry code (`indicators/orb-ema-trend-strategy.pine`): a 5-minute opening range on an index future is a small, noisy sample of the pure open-auction — likely too tight to represent a meaningful range, causing frequent low-quality breakout triggers. There's no volume/momentum confirmation on the breakout bar. The stop sits just beyond the OR high/low itself (`orHigh - buffer`), which combined with `avgBarsInLossTrade` (17.2) being close to `avgBarsInWinTrade` (13.3) suggests losses aren't fast whipsaw stop-outs — they're slow grinds the wrong way, consistent with entries that simply weren't well-timed rather than a stop placed wrong.

**Untested tonight, and why:** tried to verify parameter-tuning ideas (wider OR window, larger breakout buffer, wider stop) live via the Strategy Tester using the existing input framework — this hit two real technical snags, both worth recording:
1. `study.setInputValues()` updates the stored input value but does **not** reliably trigger `reportData()` to recalculate — confirmed by polling for 60+ seconds after a confirmed input change with zero change in the report. All prior successful backtesting in this project relied on *symbol* switches to force a recalc, never a same-symbol input change; this is a previously-unexercised code path that turned out not to work as assumed.
2. The fallback (remove the study, `chart.createStudy()` it fresh with the new inputs baked in — guaranteed to force a real calc) failed to re-create this private/custom script reliably in a second TradingView Desktop window opened for isolated testing, even though the identical call is documented working reliably in the main window (see `orb_alert.js`'s `ensureStrategyOnChart`). Root cause not isolated; left as an open problem rather than risking further disruption. The isolated second window otherwise **did** prove out as a genuine, verified-isolated chart context (a real fix for "don't flicker my main chart while automating" — confirmed a symbol switch in the second window does not propagate to the main window) — just not yet usable for strategy-input sweeps specifically.

**Recommendation for tomorrow:** don't trust this strategy's automated signal on ES1! as a real edge — it has a consistent, repeated, non-confounded negative backtest across every version tried. `orb-alert` is still watching it (Keagan asked for it to be added) and will alert on any real entry with its live win rate attached in the message, but the message itself now says so honestly when win rate is under the breakeven threshold. If trading ES1! at the NY open, the marked session levels + discretionary judgment are the safer basis than this specific automated strategy; Gold (`GC1!`) remains the only market this strategy has a verified real edge on.
