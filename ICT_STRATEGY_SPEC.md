...
| ADX/DMI trend filter | ✅ v4 (2026-07-16) | `ta.dmi(adxLen, adxSmoothing)`; not plotted directly (this script is `overlay=true`, and a 0-100 oscillator on the price scale would be unreadable) — the trend-strength reading shows as a second row in the top-right bias table, e.g. "ADX: 40.7 (Trending)" vs "(Choppy)" against `adxThreshold` (default 20). When `gateWithADX` is on (default), it also gates BOS/Order Block confirmation, so structure breaks only confirm while `ADX >= adxThreshold` — filters out chop. Toggle display via `showADX`, gating via `gateWithADX`. |
| Session VWAP | ✅ v4 (2026-07-16) | `ta.vwap(hlc3)`, session-anchored (resets daily), plotted as a fuchsia line. Toggle via `showVWAP`. |
